...
export async function* handleCoverLetter(ctx: WorkflowContext): AsyncGenerator<string, void, unknown> {
  try {
    ...
    // Step 3: Fill the textarea with validation
    const textareaResult = await ctx.driver.executeScript(`
      ...
      if (textarea) {
        ...
        // Focus and trigger validation
        textarea.focus();
        textarea.blur();

        // Check if value actually stuck
        ...
        console.log('Cover letter filled - Length:', valueLength, 'Errors:', hasErrors, 'Messages:', errorMessages);
    ...
    if (textareaResult.success) {
      ...
      if (textareaResult.hasErrors) {
        printLog(`⚠️ VALIDATION ERRORS FOUND: ${textareaResult.errorMessages.join(', ')}`);
      }
      yield "cover_letter_filled";
    } else {
      printLog(`Cover letter filling failed: ${textareaResult.error || 'Unknown error'}`);
      if (textareaResult.errorMessages) {
...
export async function* handleEmployerQuestions(ctx: WorkflowContext): AsyncGenerator<string, void, unknown> {
  try {
    ...
    const questionsData = await ctx.driver.executeScript(`
      // Find all questions on the page (ported from Python logic)
      let questions = [];
